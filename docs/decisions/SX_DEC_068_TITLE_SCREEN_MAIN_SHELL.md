...
Candidate 008 remains immutable evidence for its own pre-canonical-status package bytes, including its then-pending wordmark disposition. It is historical after the manifest promotion and must not be used for a later final-user review. Candidate 009 then bound the exact canonical-status source at `1ac3099d9ab1451323cca2935547f82d210b50b4`; it remains immutable historical evidence after SX-DEC-069 changed player-facing product bytes. `SX60-POC-ACCEPT-010` now binds the later exact source `79323ff0175b674c594d18dfd6d28a8e9951f5bd` and is the current machine-primary package. The title asset’s user pixel approval is complete, but it does not transfer physical display, device, accessibility, human/player, release, or production-cutover approval.
