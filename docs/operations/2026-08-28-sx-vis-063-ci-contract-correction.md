...
**Status:** `RESOLVED_ON_PR_244_HEAD_PENDING_REMOTE_RERUN`
...
- Base operating-contract validation and project contract validation: PASS.
...
When a protected baseline changes, update every project-local exact-hash assertion in the same review and preserve historical decision IDs required by retained evidence. `NO_BASE_PROMOTION`: this is an interaction between Switchy's legacy compatibility tests and its protected-change manifest, while Base already requires exact contract validation.
