...
> **Historical rail disposition — 2026-08-30:** After the user inspected the actual recommended board, they rejected the disconnected appearance of these four individually authored rail outputs and directed the project to build a complete connected rail image first, then derive tiles from it. `SX-VIS-063-RAIL-NETWORK-MASTER-003` and its four v03 deterministic derivatives are now the approved runtime rail family; the rail rows in this v02 record remain source/rollback history only. The terrain, locomotive, markers, stations, and cargo outputs in this record remain the active v02 slots.
