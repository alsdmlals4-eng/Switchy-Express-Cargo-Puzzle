...
Require the strict twelve-key adapter root; typed route records; executable string validators; `gdd_sheet.sync_status == "CURRENT"`; canonical baseline enums and exact PR-base commit; three-field Registry records with raw-byte SHA-256; and `docs/PROJECT_OPERATING_HEALTH.json` containing original Sheet, baseline, and validator evidence.
...
**Interfaces:**
- Consumes: exact original adapter at `main@a45176a...` and current canonical evidence paths.
...
Use exactly the twelve Base v1 root keys.
...
- [ ] **Step 4: Convert routes and validators**

Every Base route and `switchy-express-design` becomes an ACTIVE typed record. Adapter validators become executable command strings; original evidence objects remain in project health.
...
- [ ] **Step 2: Run Base and project validators**
...
- [ ] **Step 3: Reject unexpected paths and commit generated outputs**

No product, planning canon, APK, or Godot Pilot adoption file may be changed by generation.

### Task 5: Adversarial closure

- [ ] **Step 1: Prove scope and information preservation**

Confirm `project.godot`, `game/**`, `assets/**`, `기획서/**`, APK evidence, Godot Pilot adoption, and Google Sheet cells are unchanged. Every noncanonical original adapter value must have one destination.

- [ ] **Step 2: Verify exact-head checks and review state**
...
- [ ] **Step 3: Record truthful evidence and keep Draft**
