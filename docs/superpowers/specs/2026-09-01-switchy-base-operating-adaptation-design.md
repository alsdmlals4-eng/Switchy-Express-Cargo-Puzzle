...
| F4 | `PROJECT_OPERATING_HEALTH.json` exposes an early runtime/maturity snapshot without declaring it historical, although current runtime and package evidence is owned elsewhere. | `CONFLICTING_SOURCE` | Mark the file as an historical compatibility snapshot; do not raise any evidence state in it. |
