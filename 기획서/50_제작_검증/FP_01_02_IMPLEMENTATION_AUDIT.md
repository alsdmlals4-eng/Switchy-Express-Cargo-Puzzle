...
status: AUTOMATED_PASS · MANUAL_GATES_OPEN
...
그러나 자동 테스트는 실제 Android 조작성이나 처음 보는 사용자의 규칙 이해를 증명하지 않는다. 따라서 현재 상태는 제품 전환 완료가 아니라 `AUTOMATED_PASS · MANUAL_GATES_OPEN`이다.

| Gate | 상태 | 근거 | 전환 영향 |
|---|---|---|---|
| AUTOMATED | PASS | Project Contract #490, Godot Tests #451, `60 cases · 10,382 assertions · 0 failures` | 수동 검증 진행 가능 |
| ANDROID | NOT_RUN | 실기기·공식 에뮬레이터 기록 없음 | cutover 차단 |
| HUMAN | NOT_RUN | 5명 이해도 기록 없음 | cutover 차단 |
...
1. `3a4aeaa63561f78e6b1065c80bda9a64af220051`에서 별도 `validation/fp-android-smoke` 브랜치를 만든다.
2. 그 브랜치에서만 실행 진입점을 `res://game/finite/main/finite_slice.tscn`으로 바꾸는 단일 커밋을 만든다.
3. 브랜치 SHA와 APK SHA-256을 기록한다.
4. 검증 브랜치는 main에 병합하지 않는다.
5. 모든 필수 Gate PASS 후 별도의 production-cutover PR에서 같은 진입점 변경을 다시 검토한다.
...
### 3.3 Android smoke 항목

모든 항목은 `PASS`, `FAIL`, `NOT_RUN` 중 하나로 기록한다. 일부 항목만 실행한 경우 전체 Android Gate는 PASS가 아니다.
...
| AND-08 | switch while loading | LOAD 홀드와 branch 탭을 동시에 수행할 수 있음 | NOT_RUN | |
...
| AND-11 | TOP readability 8 | 8개 적재에서 rear/TOP과 순서를 구분 가능 | NOT_RUN | |
| AND-12 | TOP readability 16 | 16개 적재에서 rear/TOP과 순서를 구분 가능 | NOT_RUN | |
| AND-13 | TOP readability 32 | 32개 적재에서 rear/TOP과 순서를 구분 가능 | NOT_RUN | |
...
```text
PASS: AND-01~20 전부 PASS + 영상/스크린샷 참조 + branch SHA + APK hash 기록
FAIL: 하나 이상의 필수 항목 FAIL
NOT_RUN: 실행하지 않았거나 재현 가능한 빌드·증거가 없음
```
...
- 같은 build SHA와 같은 proof map을 사용한다.
...
다음 작업은 Android smoke와 5명 comprehension 검증이다. 이 문서가 채워지기 전에는 finite Slice를 기본 제품 런타임으로 표시하지 않는다.
