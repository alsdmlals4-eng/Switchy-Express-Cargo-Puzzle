...
- 건설 불가 지점은 바위·건물·물·보호 구역 등 세계 안의 이유와 명확한 금지 표시를 함께 사용
- 시작점·역·화물은 선로가 없어도 서로 겹치지 않고 읽혀야 함
- 카메라는 전체 맵과 핵심 오브젝트를 동시에 파악할 수 있는 가로형 구도

### 추천 설계도

- 반투명·낮은 채도의 ghost rail
- 실제 선로와 명확히 다른 재질·점선·광도
- 비용·연결 판정·열차 경로에는 포함되지 않음
- 전체 표시/숨기기 토글
...
기준을 초과해도 붉은 실패 표시가 아니라 해당 별·리더보드 자격만 놓쳤음을 보여준다.
...
- 직선·곡선: 기본 레일
- 분기: 선택 가능한 출구가 보이고 활성 방향이 밝게 연결
- 교차: 가로·세로 레일이 높이·침목·표식으로 독립임을 표시, 회전 불가
- 회차: 진행 방향이 뒤집히는 구조가 공간 형태로 이해됨
- 터널·교량: 맵이 허용한 지점에만 결합 가능

### 성능 속성

- 일반: 중립 재질
- 가속: 진행 방향 광띠·속도 표식, 과도한 상시 발광 금지
- 저비용: 간소한 침목·재질, 파손·불량처럼 보이지 않음
- 일방통행: 선로 위 반복 화살표와 진입 금지 표식
...
- 맵 위 분기를 직접 탭
- 활성 경로는 레일 밝기·레버·화살표·짧은 경로 preview로 중복 표시
...
- TOP이 역과 일치하면 역 진입 전 하역 예정 그룹 강조
- 일치하지 않으면 역이 실패 경고를 띄우지 않고 그대로 통과
- 하역은 총 최대 1초
- 화물 수가 많을수록 빠르게 연속 배출하지만 각 물체의 이동이 보임
- 화물은 뒤쪽 TOP부터 역 플랫폼·창고로 이동
- `COMBO ×N`은 하역 그룹 가까이 표시
- 하역 완료 뒤 출발 순간 가속 trail 시작
- Combo 가속과 가속 선로가 중첩될 때 효과가 과도하게 화면을 가리지 않음
- Reduced Motion에서는 카메라 흔들림 없이 속도선·UI 게이지로 대체

## 일시정지

- 제한 시간·운행 시간이 멈췄음을 명확히 표시
- 분기·적재 모드·선로 컨트롤은 비활성
- 맵과 스택은 확인 가능
- 재개 카운트다운 뒤 입력 복귀
- 랭킹 자격이 유지된다는 점을 별도 경고 없이 일관되게 처리
...
3별을 획득하면 해당 맵 리더보드가 열리는 연출을 제공하되 노선 화면을 가리지 않는다.
