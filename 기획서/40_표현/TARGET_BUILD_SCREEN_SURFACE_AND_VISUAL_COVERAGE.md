...
| --- | --- | --- | --- | --- |
| SX-VIS-063-RQ-001 board diorama | SX-SCR-004 BUILD, SX-SCR-005 route control, SX-SCR-006 RUN, SX-SCR-007 semantic events | ProductBoardRenderer product visual asset paths | AUTOMATED_MACHINE_RUNTIME_LOCAL_PACKAGE_AND_HOSTED_CI_VERIFIED | User-approved v02 terrain plus nine non-rail Core Board assets and four user-approved v03 rail-master derivatives are connected to the existing 14 slots. The v03 rails come from a preserved single connected master and use no procedural seam underlay; actual recommended/ghost BUILD captures verify the consumer. Rebuilt Windows/Android PCK proofs retain every v03 runtime rail and exclude the derivation master/review captures; all seven hosted checks passed for exact runtime-byte head `c07e02b`. Exact GitHub candidate and human/device gates remain open. |
