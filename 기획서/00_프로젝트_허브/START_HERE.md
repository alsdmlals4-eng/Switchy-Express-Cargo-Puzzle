...
PR312/main5309829b, with eighteen optional stages. D2 diagnostics are merged in
PR313/main a82798d0; original intermittent cause remains unconfirmed. Current bounded
preparation is the unchanged package's file/rights coverage inventory, not release execution;
see Active Context and docs/superpowers/plans/2026-09-14-shipping-inventory.md.
...
| post-060 candidate route | `evidence/acceptance/post_sx_dec_060_candidate.json · SX60-POC-ACCEPT-010 is the exact post-SX-DEC-069 transparent-wayside/speed-transition machine package · Candidates 001–009 are historical · FINAL_USER_REVIEW_NOT_RUN · TITLE_WORDMARK_USER_PIXEL_APPROVED_CANON_REGISTERED · V02_WAYSIDE_USER_PIXEL_REVIEW_PENDING` |
...
| post-060 Windows physical | `FINAL_USER_REVIEW_ONLY · unchanged SX60-POC-ACCEPT-010 required · NOT_RUN` |
...
Do not run its old physical Gate 0 as if it validated the new rule. `SX60-POC-ACCEPT-006` is historical Route Book 01 evidence, `SX60-POC-ACCEPT-007` is historical post-SX-DEC-067 evidence, Candidate 008 is historical pre-canonical-wordmark-status evidence, and `SX60-POC-ACCEPT-009` is historical pre-SX-DEC-069 evidence; `SX60-POC-ACCEPT-010` is the exact post-SX-DEC-069 machine-primary package and remains exact-byte-specific.
...
→ SX-DEC-068 changed player-facing title-shell bytes; Candidate 008 remains historical pre-canonical-wordmark-status evidence, and SX-DEC-069 changed transparent wayside/speed-transition bytes, making Candidate 009 historical while Candidate 010 binds the exact current package
→ FINAL_USER_REVIEW only if the user requests final inspection of unchanged Candidate 010 bytes; title wordmark pixel approval and canonical registration are complete while v02 wayside pixel review remains pending
