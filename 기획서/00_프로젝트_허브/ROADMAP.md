...
Negative no-pickup rejects; no product scope expansion. PR308 owns exact integration readback.
...
SX-DEC-056~058
→ additive product-depth planning exists
→ implementation remains unauthorized/blocked as documented

SX-DEC-059
→ release-near first session implemented/merged
→ pre-060 playable POC + Candidate 003 package evidence preserved
...
SX-DEC-067 · USER_APPROVED · MERGED_MAIN_VERIFIED · PR #263 · main@c0bb86e · Candidate_007_historical_after_SX_DEC_068
...
contiguous matching TOP unload
finite timer/outcome/retry
```

Changed by SX-DEC-060:

```text
station exact-cell contact → cardinal adjacent service
all-required-anchor interpretation → start-reachable cargo + station-service coverage
irrelevant disconnected rail island → allowed
```
...
Pipeline evidence remains useful, but its gameplay bytes do not prove SX-DEC-060.
...
station_png_consumer: ProductBoardRenderer
sx_dec_060_new_bitmap_requirement: 0
```

SX-DEC-060 reuses existing station PNGs and adds procedural service indication first.

## M5 · SX-DEC-055 Runtime Semantic POC

`IMPLEMENTED · MERGED_MAIN_VERIFIED · HISTORICAL_FOUNDATION`

No new gameplay/scoring/save authority is widened by retaining this evidence.

## M5A · Product-depth packages held outside current implementation
...
R09 Shareable Route Card → POST_VALIDATION_HOLD
R10 Editor/UGC → POST_VALIDATION_HOLD
```

## M5B · SX-DEC-059 Release-Near First Session

`MERGED_MAIN_VERIFIED · PRE_SX_DEC_060_PRODUCT_EVIDENCE`

Flow remains current in shape:
...
```yaml
candidate: SX59-POC-ACCEPT-003
package_integrity: PASS
pck_integrity: PASS · 472/472
product_textures: PASS · 73/73
physical_visual_recheck: NOT_RUN
role_after_sx_dec_060: HISTORICAL_EXACT_BYTES_ONLY
```
...
not every placed rail piece must join one global network
```
...
required_coverage: ALL_CARGO + >=1_CARDINAL_SERVICE_CELL_PER_STATION
irrelevant_disconnected_island: ALLOWED
...
→ Candidate 006 is historical; SX-DEC-068 later changed title-shell bytes, so Candidate 007 is historical; Candidate 008 is historical pre-canonical-wordmark-status evidence; SX-DEC-069 then changed transparent wayside/speed-transition bytes, so Candidate 009 is historical and Candidate 010 binds the exact current machine package
```
...
POST_060_PACKAGE: SX60-POC-ACCEPT-010_PREPARED_PACKAGE_VERIFIED · HISTORICAL_SX60_POC_ACCEPT_001_TO_009_PRESERVED · TITLE_WORDMARK_USER_PIXEL_APPROVED_CANON_REGISTERED · V02_WAYSIDE_USER_PIXEL_REVIEW_PENDING
...
`MACHINE_PRIMARY_FINAL_USER_REVIEW · USER_APPROVED · SX60-POC-ACCEPT-010_PREPARED_PACKAGE_VERIFIED · FINAL_USER_REVIEW_NOT_RUN · TITLE_WORDMARK_USER_PIXEL_APPROVED_CANON_REGISTERED · V02_WAYSIDE_USER_PIXEL_REVIEW_PENDING`

The v04 runtime-byte change retains immutable exact Candidate 005 historical evidence. Route Book 01 subsequently minted Candidate 006, which SX-DEC-067 then superseded with player-facing bytes. SX-DEC-068 subsequently changed the title shell and made Candidate 007 historical. Candidate 008 bound those title-shell bytes before the user-approved manifest promotion; that status promotion required Candidate 009. SX-DEC-069 then changed transparent wayside/speed-transition player-facing bytes, so Candidate 009 is historical and Candidate 010 now binds the exact current package. The current sequence is:

```text
Candidate 008 immutable exact pre-canonical-wordmark-status package verification retained
→ Candidate 009 immutable exact post-canonical-wordmark-status package verification retained as pre-SX-DEC-069 history
→ Candidate 010 immutable exact post-SX-DEC-069 package verification complete
→ MACHINE_PRIMARY_ACCEPTANCE_READY
→ optional FINAL_USER_REVIEW only when requested on unchanged Candidate 010; title wordmark pixel approval and canonical registration are complete while v02 wayside pixel review remains pending
```
...
Requires a separate decision after post-060 release-near evidence. No merge/build/export result automatically authorizes store release.

## Holds / exclusions

```text
endless/fuel/BOOST/capacity-8/cargo-slowdown/pickup-respawn/switch-auto-reset → NON_CURRENT
arbitrary station radius / diagonal service / multi-station priority → OUT_OF_SCOPE
new explanatory/mockup bitmap without game consumer → OUT_OF_SCOPE
