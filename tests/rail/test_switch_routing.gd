...
func run() -> void:
	...
	assert_equal(two_state.current_exit(), right, "two-state switch must prefer straight travel as route A")
	two_state.cycle_state()
	assert_equal(two_state.current_exit(), up, "two-state switch must cycle A to B")
	two_state.cycle_state()
	assert_equal(two_state.current_exit(), right, "two-state switch must cycle B to A")
	...
	assert_equal(three_state.current_exit(), right, "three-state switch must prefer straight travel as route A")
	three_state.cycle_state()
	assert_equal(three_state.current_exit(), up, "three-state switch must cycle A to B")
	three_state.cycle_state()
	assert_equal(three_state.current_exit(), down, "three-state switch must cycle B to C")
	three_state.cycle_state()
	assert_equal(three_state.current_exit(), right, "three-state switch must cycle C to A")
	...
	assert_equal(three_state.current_exit(), right, "switch must reset to its straight default route after passage")

	var vertical_approach: Variant = switch_script.new()
	vertical_approach.configure(center, [left, up, right, down], up)
	assert_equal(vertical_approach.current_exit(), down, "straight route must also be preferred for vertical approaches")
