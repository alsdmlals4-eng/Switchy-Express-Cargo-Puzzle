...
func run() -> void:
	...
	assert_not_equal(session.graph.next_cell(switch_cell, switch_approach), authored_exit, "preconfigured switch must change its selected exit")
	assert_true(session.graph.cycle_switch(switch_cell), "second cycle must restore a two-state switch")
	assert_equal(session.graph.next_cell(switch_cell, switch_approach), authored_exit, "second cycle must restore authored exit")
