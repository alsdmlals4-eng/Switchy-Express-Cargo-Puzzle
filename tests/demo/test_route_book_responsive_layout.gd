...
func run() -> void:
	...
	for viewport_size: Vector2 in VIEWPORT_SIZES:
		...
		assert_equal(list.get_child_count(), 3, "Route Book keeps three book cards at %s" % viewport_size)
		...
		assert_true(demo.select_route_book(&"ROUTE_BOOK_01"), "Route Book 01 selection works at %s" % viewport_size)
		_force_layout(demo)
		assert_equal(list.get_child_count(), 6, "selected Route Book keeps six direct cards at %s" % viewport_size)
		assert_true(demo.select_route_book_stage(&"RB02_REVERSE_ORDER"), "Route Book stage selection works at %s" % viewport_size)
		demo.begin_build()
