...
func run() -> void:
	...
	var body := demo.get_node("ResultOverlay/Panel/Content/BodyScroll/Body") as Label
	for marker: String in ["경과 시간", "남은 시간", "건설비", "하역"]:
		assert_true(body.text.contains(marker), "result summary must include %s" % marker)

	for path: NodePath in [
		NodePath("ResultOverlay/Panel/Content/Actions/RetryButton"),
		NodePath("ResultOverlay/Panel/Content/Actions/EditButton"),
		NodePath("ResultOverlay/Panel/Content/Actions/TitleButton"),
	]:
		assert_not_null(demo.get_node_or_null(path), "%s must exist" % path)
