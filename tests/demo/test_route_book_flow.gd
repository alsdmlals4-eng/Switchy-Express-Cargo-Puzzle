...
func run() -> void:
	...
	assert_not_null(stage_list, "Route Book has a dedicated six-card stage list")
	if stage_list != null:
		assert_equal(stage_list.get_child_count(), 6, "all six Route Book stages are directly selectable")
	assert_false(demo.select_route_book_stage(&"UNKNOWN"), "unknown Route Book stage is rejected")
	assert_true(demo.select_route_book_stage(&"RB03_RETURN_MANIFEST"), "known card selects Route Book stage")
	assert_equal(demo.state(), &"BRIEFING", "selected Route Book stage reuses briefing")
	assert_equal(demo.current_route_book_stage_id_for_test(), &"RB03_RETURN_MANIFEST", "selected stage identity remains exact")
	demo.begin_build()
	...
	assert_equal(demo.current_route_book_stage_id_for_test(), &"RB04_LOAD_WINDOW", "Next Stage follows declared order")
	demo.begin_build()
	demo.show_result({"outcome": &"SUCCESS"})
	if stage_book != null:
